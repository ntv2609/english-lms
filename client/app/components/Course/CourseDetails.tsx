          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-6 pt-4 border-b border-black/5 dark:border-white/5 pb-8">
              <div className="flex items-center gap-2">
                ...
                <span className="font-mono text-sm font-bold text-black dark:text-white">{data?.ratings?.toFixed(1) || 0}</span>
                <span className="text-sm text-neutral-500">({data?.reviews?.length || 0} đánh giá)</span>
              </div>
              <div className="w-1 h-1 rounded-full bg-neutral-300 dark:bg-neutral-700 hidden sm:block"/>
              <span className="text-sm font-medium text-black dark:text-white">{data?.purchased || 0} Học viên</span>
            </div>
          </div>
              ...
              {(data?.reviews && [...data.reviews].reverse()).map((item: any, index: number) => (
                <div key={index} className="p-6 bg-white dark:bg-[#0A0A0A] border border-black/5 dark:border-white/5 rounded-2xl shadow-sm flex gap-4">
                  <div className="w-12 h-12 bg-black dark:bg-white rounded-full flex items-center justify-center shrink-0">
                    <span className="font-Josefin font-bold text-xl text-white dark:text-black uppercase">{item.user?.name ? item.user.name.charAt(0) : "U"}</span>
                  </div>
                  <div>
        ...
        <div className="lg:col-span-1">
          <div className="sticky top-24 bg-white dark:bg-[#0A0A0A] border border-black/10 dark:border-white/10 rounded-2xl overflow-hidden shadow-2xl p-6">
            <div className="aspect-video bg-neutral-900 rounded-lg overflow-hidden mb-6 border border-white/10">
               <CoursePlayer videoUrl={data?.demoUrl} title={data?.title || data?.name} />
            </div>
            <div className="flex flex-wrap items-end gap-3 mb-6">
              <span className="text-4xl font-Josefin font-bold tracking-tight text-black dark:text-white">
                {data?.price === 0 || !data?.price ? "Miễn phí" : `${data.price.toLocaleString('vi-VN')} VNĐ`}
              </span>
              {data?.estimatedPrice && (
                <span className="text-lg text-neutral-400 line-through mb-1">
                  {data.estimatedPrice.toLocaleString('vi-VN')} VNĐ
                </span>
              )}
              {discountPercentage > 0 && (
                <span className="px-2 py-1 bg-red-500/10 text-red-500 font-bold text-xs rounded uppercase tracking-wider mb-1.5">
            ...
            {mounted && isPurchased ? (
              ...
              <button disabled={isLoading} onClick={handleOrder} className={`${styles.button} !w-full !rounded-xl !py-4 !bg-black dark:!bg-white dark:!text-black ${isLoading ? "opacity-50" : "hover:-translate-y-1"}`}>
                {isLoading ? "Đang xử lý thanh toán..." : `Đăng ký khóa học ${data?.price ? data.price.toLocaleString('vi-VN') + ' VNĐ' : ''}`}
              </button>
            )}
