...
const CoursePreview: FC<Props> = ({ courseData, handleCourseCreate, active, setActive, isEdit }) => {
  ...
  return (
    <div className="w-full max-w-5xl mx-auto mt-10 space-y-10 px-2 sm:px-0">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        <div className="lg:col-span-2 space-y-8">
          <div>
            <h1 className="text-2xl sm:text-3xl font-Josefin font-bold text-black dark:text-white leading-tight">{courseData?.name}</h1>
            <p className="mt-4 text-neutral-600 dark:text-neutral-400 whitespace-pre-line leading-relaxed text-sm sm:text-base">{courseData?.description}</p>
          </div>
          ...
          <div className={styles.card + " p-6"}>
            ...
            <div className="space-y-3">
              {courseData?.benefits?.map((b:any, i:number) => (
                <div key={i} className="flex gap-3 text-neutral-700 dark:text-neutral-300 text-sm sm:text-base"><IoCheckmarkDoneOutline className="text-emerald-500 shrink-0 mt-1" size={20}/><span>{b.title}</span></div>
              ))}
            </div>
          </div>
          ...
          <div className={styles.card + " p-6"}>
            ...
            <div className="space-y-3">
              {courseData?.prerequisites?.map((p:any, i:number) => (
                <div key={i} className="flex gap-3 text-neutral-700 dark:text-neutral-300 text-sm sm:text-base"><span className="text-blue-500 shrink-0 mt-1 font-mono">0{i+1}.</span><span>{p.title}</span></div>
              ))}
            </div>
          </div>
        </div>
          ...
          <div className="sticky top-24 bg-white dark:bg-[#0A0A0A] border border-black/10 dark:border-white/10 rounded-xl overflow-hidden shadow-2xl">
            ...
            <div className="p-6 space-y-6">
              <div className="flex flex-wrap items-end gap-3">
                <h1 className="text-2xl sm:text-3xl font-Josefin font-bold text-black dark:text-white">${courseData?.price || "Free"}</h1>
      ...
      <div className="flex flex-col-reverse sm:flex-row justify-between items-center gap-4 pt-10 border-t border-black/5 dark:border-white/5">
        ...
        <button className={`${styles.button} !w-full sm:!w-48`} onClick={() => handleCourseCreate()}>{isEdit ? "Cập nhật khóa học" : "Đăng tải khóa học"}</button>
      </div>
    </div>
  );
};
