...
const EditHero: FC = () => {
  ...
  return (
    <div className="max-w-5xl m-auto px-2 sm:px-0">
      <div className="mb-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
        <div className="space-y-6">
          <div className={styles.card + " p-6"}>
            ...
            <textarea
              className="w-full bg-transparent text-2xl sm:text-3xl font-Josefin font-bold text-black dark:text-white outline-none resize-none border-b border-black/10 dark:border-white/10 focus:border-blue-500 pb-2 transition-colors"
          ...
          <div className={styles.card + " p-6"}>
            ...
            <textarea
              className="w-full bg-transparent text-sm sm:text-base text-neutral-600 dark:text-neutral-400 outline-none resize-none border-b border-black/10 dark:border-white/10 focus:border-blue-500 pb-2 transition-colors"
              value={subTitle} onChange={(e) => setSubTitle(e.target.value)} rows={3} placeholder="Mô tả chi tiết..."
            />
          </div>
          <button
            className={`${styles.button} !w-full sm:!w-40 ${data?.layout?.banner?.title === title && data?.layout?.banner?.subTitle === subTitle && data?.layout?.banner?.image?.url === image ? "opacity-50 cursor-not-allowed" : ""}`}
        ...
        <div className="relative group mt-8 lg:mt-0">
          <div className={styles.card + " p-2 aspect-[4/3] relative flex items-center justify-center overflow-hidden"}>
             ...
             <div className="absolute inset-0 bg-black/40 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center z-20 backdrop-blur-sm">
