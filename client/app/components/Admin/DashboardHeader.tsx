...
const DashboardHeader: FC<Props> = ({ open, setOpen }) => {
  ...
  const [audio] = useState<any>(
    typeof window !== "undefined"
      ? new Audio("https://res.cloudinary.com/damk251/video/upload/v1693465789/notification_vcetjn.mp3")
      : null
  );

  const playNotificationSound = () => {
    if (audio) {
      audio.play().catch((error: any) => console.log("Audio play error:", error));
    }
  };
  ...
  const handleNotificationStatusChange = async (id: string) => {
    await updateNotificationStatus(id.toString());
  };
  ...
  return (
    <div className="w-full flex items-center justify-end p-6 fixed top-5 right-0 z-[9999999]">
      ...
      {open && (
        <div className="w-[350px] h-[50vh] dark:bg-[#111C43] bg-white shadow-xl absolute top-16 z-10 rounded overflow-y-scroll">
          ...
          {notifications && notifications.map((item: any, index: number) => (
            <div key={index} className="dark:bg-[#2d3a4ea1] bg-[#00000013] font-Poppins border-b dark:border-b-[#ffffff47] border-b-[#0000000f]">
              ...
              <p className="px-2 text-black dark:text-white text-[14px]">{item.message}</p>
